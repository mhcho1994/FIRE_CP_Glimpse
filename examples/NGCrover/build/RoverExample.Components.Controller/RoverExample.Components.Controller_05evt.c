...
void RoverExample_Components_Controller_function_initSample(DATA *data, threadData_t *threadData)
{
  ...
  data->modelData->samplesInfo[i].interval = (data->simulationInfo->realParameter[data->simulationInfo->realParamsIndex[4]] /* sample_interval PARAM */) /* (max real for single time events) */;
  i++;
}
